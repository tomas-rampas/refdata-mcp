...
param(
    ...
    [string]$OutputDirectory = ".confjson",
...
function Get-DocumentationLinks {
    ...
    # XPath to find all links within the primary navigation menu
    $xpath = "//nav[@aria-label='Primary navigation']//a[@href]"
...
function Get-PageContent {
    ...
    if (-not $contentNode) {
        Write-Warning "Could not find main content div on page $PageUrl"
        return $null
    }
    ...
    $title = ($contentNode.SelectSingleNode("//h1") | Select-Object -ExpandProperty InnerText).Trim()
...
function Save-PayloadToFile {
    ...
    $fileName = $sanitizedTitle + ".confjson"
